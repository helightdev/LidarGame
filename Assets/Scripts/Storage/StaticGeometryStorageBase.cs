...
namespace Storage {
    public class StaticGeometryStorageBase : PointStorageBase {
        ...
        [DoNotSerialize, HideInInspector] public bool dirty;
        ...
        public override void UpdatePlayerPosition(float3 position) {
            ...
            foreach (var entry in _vfxEntries) entry.effect.SetVector3(_referencePosParamName, position);
        }
        ...
        private async UniTaskVoid InsertionTask() {
            ...
            try {
                var insertions = new NativeArray<PointInsertData>(_scheduledInsertData.Length, Allocator.Persistent);
                var temp = new NativeArray<PointInsertData>(_scheduledInsertData.Length, Allocator.Persistent);
